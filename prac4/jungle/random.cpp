...
inline bool rat_blocking1(int x, int y, int z) {
    return 
        (x != 0) ||
        (y != 0) ||
        (z != 0);
}
inline bool rat_blocking2(int x, int y) {
    return
        (x != 0) ||
        (y != 0);
}
    ...
    for(auto& piece : player.pieces) {
        ...
        for(auto& new_directions : directions) {
            ...
            if((piece.typ == 6 || piece.typ == 7) && is_pond(nrow, ncol)) {
                if(new_directions.first != 0) {
                    int x = s.tab[nrow][ncol];
                    int y = s.tab[nrow + new_directions.first][ncol];
                    int z = s.tab[nrow + 2 * new_directions.first][ncol];
                    if(!rat_blocking1(x, y, z))
                        nrow += 3 * new_directions.first;
                } else if(new_directions.second != 0) {
                    int x = s.tab[nrow][ncol];
                    int y = s.tab[nrow][ncol + new_directions.second];
                    if(!rat_blocking2(x, y))
