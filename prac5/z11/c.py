...
def actions(s : State) -> List[Action]:
    return [Action(i, j) for i in range(-1, 2) for j in range(-1, 2)]
...
def possible_states(s : State, action : Action):
    if track[s.y][s.x] != 'o':
        ...
        for (oilx, oily) in directions:
            new_action = Action(action.dvx + oilx, action.dvy + oily)
            yield transition(s, new_action)
            ...
            for action in actions(s):
                ...
                for t in possible_states(s, action):
                    if not isinstance(t, State):
                        ...
                        cur_value += \
                            prob(s, action, t) * reward(s, action, t)
                    else:
                        cur_value += \
                            prob(s, action, t) * (reward(s, action, t) + GAMMA * values[t])
                if cur_value > max_value:
                    max_value = cur_value
            if abs(max_value - values[s]) > eps:
                changed = True
...
def optimal_policy(values) -> List[str]:
    ...
    all_states = get_all_states()
    for s in all_states:
        max_value, arg_max = -1e6, Action(0, 0)
        for action in actions(s):
            cur_value = 0
            for t in possible_states(s, action):
                if not isinstance(t, State):
