...
struct ZHash {
    ...
    void calc_zhash(State &s) {
        ...
        for(int i = 0; i < 9; i++) {
            for(int j = 0; j < 7; j++) {
                if(s.tab[i][j] != 0) res ^= hashes[i][j][s.tab[i][j] - 1];
            }
        }
        s.hash = res;
    }
    void zhash_move(State &s, int id, int row1, int col1, int row2, int col2) {
        s.hash ^= hashes[row1][col1][id - 1];
        s.hash ^= hashes[row2][col2][id - 1];
    }
    void zhash_remove(State &s, int id, int row1, int col1) {
        s.hash ^= hashes[row1][col1][id - 1];
    }
...
void change_state(State &s, Move m) {
    ...
    if(s.tab[m.row2][m.col2] != 0) {
        zhash.zhash_remove(s, s.tab[m.row2][m.col2], m.row2, m.col2);
        int cnt2 = 0;
    ...
    zhash.zhash_move(s, m.piece_id, m.row1, m.col1, m.row2, m.col2);
    zhash.zhash_turn(s);
...
int piece_values[8] = {120, 100, 100, 110, 120, 125, 135, 150};
// return 1/0/-1 if won/not resolved/lost by player
int is_fin(State &state, int player) {
    if(state.p[player].pieces.size() == 0) {
        return -1;
    }
    if(state.p[player ^ 1].pieces.size() == 0) {
        return 1;
    }
    if(state.p[player].pieces[0].id <= 8) {
        if(state.tab[8][3] != 0) {
            return -1;
        } else if(state.tab[0][3] != 0) {
            return 1;
        } else {
            return 0;
        }
...
// 1 - MAX, 0 - MIN
pair<int, Move> alphabeta(State &state, int depth, int alpha, int beta, int player, int our_player) {
    auto it = zhashes.find(state.hash);
    if(it != zhashes.end()) {
        return (*it).second;
    }
    int result = is_fin(state, our_player);
    if(result != 0) {
        return {result * WIN, {}};
    }
    if(depth == max_depth) {
    ...
    for(int i = 0; i < (int)moves.size(); i++) {
        ...
        pair<int, Move> p = alphabeta(s, depth + 1, alpha, beta, 1 - player, our_player);
        if(player == 1) {
            if(p.first > beta) {
    ...
    zhashes[state.hash] = {best_score, res};
    return {best_score, res};
}
Move choose_move(State &state) {
    return alphabeta(state, 0, -INF, INF, 1, state.cur).second;
}
